...
function ConsoleUtil(monitor) {
  monitor.on('new-outage', eventHandlers.onNewOutage);
  monitor.on('current-outage', eventHandlers.onCurrentOutage);
  monitor.on('service-error', eventHandlers.onFailedCheck);

  monitor.on('latency-warning', eventHandlers.onLatencyWarning);
  monitor.on('service-back', eventHandlers.onServiceBack);
  monitor.on('service-ok', eventHandlers.onServiceOk);
}

exports = module.exports = ConsoleUtil;